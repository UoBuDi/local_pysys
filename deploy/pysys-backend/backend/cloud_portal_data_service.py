...
class CloudPortalDataService:
    def __init__(self, access_token: str, session_id: str = None):
    ...
    def get_road_sections(self, center_no: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                url,
                json={"session_id": self.session_id, "center_no": center_no},
                timeout=30
            )
            response.raise_for_status()
    ...
    def get_gantry_list(self, road_section_no: str) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                url,
                json={"session_id": self.session_id, "road_section_no": road_section_no},
                timeout=30
            )
            response.raise_for_status()
...
def run_cloud_portal_data_sync(access_token: str, session_id: str = None, progress_callback=None) -> Dict[str, Any]:
    service = CloudPortalDataService(access_token, session_id)
