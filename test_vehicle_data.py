...
def main():
    ...
    try:
        ...
        # 获取2024-05月份的所有唯一hourBatchNo，只取前5个
        ...
        print(f"共发现 {total_batches} 个测试批次")
        ...
        # 测试查看已匹配结果
        print(f"\n1. 查看已匹配结果 ({total_matched} 条)")
        if all_results['matched']:
            print("前5条记录示例：")
            for i, record in enumerate(all_results['matched'][:5]):
        ...
        # 测试查看未匹配结果
        print(f"\n2. 查看未匹配结果 ({total_unmatched} 条)")
        if all_results['unmatched']:
            print("前5条记录示例：")
            for i, record in enumerate(all_results['unmatched'][:5]):
