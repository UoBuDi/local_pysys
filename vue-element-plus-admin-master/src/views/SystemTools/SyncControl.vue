...
const connectWebSocket = () => {
  // 构建WebSocket URL（使用与API相同的端口）
  const wsProtocol = API_BASE_URL.startsWith('https') ? 'wss' : 'ws'
  const wsHostname = new URL(API_BASE_URL).hostname
  const wsPort = new URL(API_BASE_URL).port || (API_BASE_URL.startsWith('https') ? '443' : '80')

  // 同步进度WebSocket
  syncProgressWs = new WebSocket(`${wsProtocol}://${wsHostname}:${wsPort}/ws/sync-progress/`)
  ...
  // 日志WebSocket
  logsWs = new WebSocket(`${wsProtocol}://${wsHostname}:${wsPort}/ws/logs/`)
