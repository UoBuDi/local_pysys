  ...
  >
    ...
    <div v-else-if="detail" style="display: flex; gap: 16px; height: 82vh; overflow: hidden">
      <div class="order-detail-left">
        <div class="order-detail-section">
          ...
          >
            <el-descriptions-item label="车牌号码">{{
              detail.labelVo?.vehicle_no || '-'
            }}</el-descriptions-item>
            <el-descriptions-item label="入口收费站">{{
              detail.labelVo?.en_station_name || '-'
            }}</el-descriptions-item>
            <el-descriptions-item label="入口时间">{{
              detail.labelVo?.en_time || '-'
            }}</el-descriptions-item>
            <el-descriptions-item label="出口收费站">{{
              detail.labelVo?.ex_station_name || '-'
            }}</el-descriptions-item>
            <el-descriptions-item label="出口时间">{{
              detail.labelVo?.ex_time || '-'
            }}</el-descriptions-item>
            <el-descriptions-item label="出口收费总额">
              <span style="color: #e6a23c; font-weight: bold"
                >¥ {{ detail.pay_amount ? (detail.pay_amount / 100).toFixed(2) : '0.00' }} 元</span
              >
            </el-descriptions-item>
          </el-descriptions>
        </div>
        ...
        <div
          v-if="detail.labelCodeList && detail.labelCodeList.length > 0"
        ...
        >
          ...
          >
            ...
            <div v-if="label.auditMethod" class="order-detail-label-method">{{
              label.auditMethod
            }}</div>
          </div>
        </div>
        ...
        <div
          v-if="detail.auditCheckdescConfigs && detail.auditCheckdescConfigs.length > 0"
          class="order-detail-section"
        >
          <div class="order-detail-section-title">✅ 稽核步骤</div>
          <div
            v-for="(config, idx) in detail.auditCheckdescConfigs"
            :key="idx"
          ...
          <div class="order-detail-image-list">
            ...
            >
              ...
              <div style="flex: 1; min-width: 0">
                <div class="order-detail-image-name">{{
      ...
      <div class="order-detail-right">
        <div style="margin-bottom: 20px">
          ...
          <div
            v-if="detail.labelCodeList && detail.labelCodeList.length > 0"
            class="order-detail-tags-section"
          >
        ...
        <div v-if="selectedPicture">
          ...
          <div
            ...
            class="order-detail-small-images"
          >
        ...
        <div v-else class="order-detail-empty-pictures">
          ...
          <div style="margin-top: 8px; font-size: 13px"
            >共
            {{ detail.picBeanVo?.total || detail.picBeanVo?.picBeanList?.length || 0 }} 张图片</div
          >
        </div>
