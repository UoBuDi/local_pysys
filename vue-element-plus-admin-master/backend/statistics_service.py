...
from core.db import get_user_db_connection, get_check_data_connection, get_db_config
...
# 读取 config.ini 中 PATH_MATCH 节的配置
...
def get_table_name() -> str:
    return _config.get('PATH_MATCH', 'table_name', fallback='202005-202311_cf_1215')
...
def start_task_execution(task_name: str) -> int:
    """记录任务开始执行到 task_execution_history，返回 history_id"""
    conn = get_user_db_connection()
...
def end_task_execution(history_id: int, status: str, message: str, details: dict = None, duration: int = None):
    """更新 task_execution_history 记录的执行结果"""
    conn = get_user_db_connection()
...
def get_consecutive_failures(task_name: str) -> int:
    """查询任务连续失败次数（从执行历史中倒序统计）"""
    conn = get_user_db_connection()
    try:
        with conn.cursor() as cursor:
            cursor.execute("""
                SELECT status FROM task_execution_history
                WHERE task_name = %s
